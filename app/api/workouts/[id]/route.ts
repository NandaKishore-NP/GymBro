  ...
  try {
    ...
    if (isProduction) {
      try {
          ...
          if (body.exercises && Array.isArray(body.exercises)) {
            ...
            if (exerciseIds.length > 0) {
              ...
              await mysqlDb.query(`
                ...
                WHERE workout_id = ? AND id NOT IN (${placeholders})
            ...
            for (const exercise of body.exercises) {
              ...
              if (exercise.id) {
                ...
                await mysqlDb.update(`
                  UPDATE exercises
                  SET name = ?, sets = ?, reps = ?, weight = ?, updated_at = NOW()
        ...
        console.error("MySQL error updating workout:", error);
        return NextResponse.json(
  ...
  try {
    ...
    if (isProduction) {
      try {
        ...
        await mysqlDb.update(`DELETE FROM workouts WHERE id = ?`, [workoutId]);
        
        return NextResponse.json({ message: "Workout deleted successfully" });
      } catch (error) {
        console.error("MySQL error deleting workout:", error);
        return NextResponse.json(
